...
namespace LuaSTGEditorSharp.EditorData.Node.Advanced.AdvancedRepeat
{
    ...
    public class SinusoidalOscillationVariable : VariableTransformation
    {
        ...
        {
            string sineTokenHead = $"({Macrolize(3)}-({Macrolize(2)}))/2*sin";
            string sineTokenTail = $"+({Macrolize(3)}+({Macrolize(2)}))/2";
            string begin = $"local {NonMacrolize(0)}={sineTokenHead}({Macrolize(1)}){sineTokenTail}"
