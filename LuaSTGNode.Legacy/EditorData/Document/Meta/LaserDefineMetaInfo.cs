...
{
    public class LaserDefineMetaInfo : MetaInfoWithDifficulty, IComparable<LaserDefineMetaInfo>
    {
        LaserInit Init { get; set; }

        public LaserDefineMetaInfo(LaserDefine target) : base(target) { }

        private void TryChild()
        {
            foreach (TreeNodeBase t in this.target.GetLogicalChildren())
            {
                if (t is LaserInit) Init = t as LaserInit;
            }
        }
        ...
        public int CompareTo(LaserDefineMetaInfo other)
        {
        ...
        {
            documentMetaData.aggregatableMetas[(int)MetaType.Laser].Add(meta);
        }

        public override void Remove(IAggregatableMeta meta, MetaDataEntity documentMetaData)
        {
            documentMetaData.aggregatableMetas[(int)MetaType.Laser].Remove(meta);
        }
        ...
        public override MetaModel GetFullMetaModel()
        {
            ...
            {
                Icon = "/LuaSTGNode.Legacy;component/images/16x16/laserdefine.png",
                Text = Name
            };
